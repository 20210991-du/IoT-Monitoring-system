...
def main():
    ...
    # test 구간 데이터 저장 → predict에서 전체 엑셀 재로드 없이 미관측 구간만 사용
    test_data_path = os.path.join(save_dir, 'test_data.parquet')
    test_df.to_parquet(test_data_path, index=False)
    print(f'  - test_data.parquet 저장 완료 ({len(test_df):,}행, 미관측 15% 구간)')
    ...
    print('  * test_data.parquet  (미관측 test 15% 구간 → predict에서 사용)')
    if thresholds:
