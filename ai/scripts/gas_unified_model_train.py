...
==========================
[통합 학습용] 전체 장비 공통 LSTM AutoEncoder 모델을 학습합니다.

  데이터 로드  ──▶  공통 LSTM 학습  ──▶  완료
  (DB)               (BASE_FEATURES)
...
class TrainingConfig:
    ...
    def all_raw_features(self) -> List[str]:
        return list(self.base_features) + [self.comm_quality_col]
...
def build_master_dataset_from_db(
    ...
    반환 DataFrame:
        측정시각, 장비번호, 방식전위, AC유입, 온도, 습도, 통신품질,
...
def inject_faults(X: np.ndarray, fault_ratio: float = 0.05, seed: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    ...
    spike/drop: 마지막 25% step 전체에 주입 (1 step 주입 시 희석되는 문제 방지)
    """
    np.random.seed(seed)
    X_fault = X.copy()
    ...
    for idx in indices:
        ...
        feat_idx   = np.random.randint(0, n_feats)

        if fault_type == 'spike':
            X_fault[idx, -tail:, feat_idx] += np.random.uniform(0.5, 0.8)
        elif fault_type == 'drop':
            X_fault[idx, -tail:, feat_idx] -= np.random.uniform(0.5, 0.8)
        elif fault_type == 'drift':
            drift = np.linspace(0, np.random.uniform(0.3, 0.5), n_steps)
            X_fault[idx, :, feat_idx] += drift

    return X_fault, y_true


def tune_threshold_f1(model, X_test: np.ndarray, fault_ratio: float = 0.15) -> Tuple[float, Dict[str, float]]:
    """
    ...
    # p50~p99.9 범위 탐색 (기존 p90~p99.9에서 확장 — 희생양극처럼 MSE 분포가 낮은 모델 대응)
    search_thrs = np.percentile(mse, np.linspace(50, 99.9, 100))
...
def main(config: TrainingConfig = DEFAULT_CONFIG):
    ...
    print()
    print(f'  [설계 정책]')
    print(f'    입력 피처: {list(config.base_features)} → {len(c_feats)}개 피처')
    print(f'    스케일러 : 장비 형식 기반 그룹 정규화')
