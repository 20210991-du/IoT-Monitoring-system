...
-- 자가확장 챗봇 — LLM 이 audit_log 분석해서 제안한 도구 후보 (운영자 승인 후 활성)
-- 승인된 도구는 서버 시작 시 / reload API 호출 시 TOOLS 배열에 동적 등록.
CREATE TABLE IF NOT EXISTS tool_registry (
  id                       BIGINT AUTO_INCREMENT PRIMARY KEY,
  name                     VARCHAR(64) NOT NULL UNIQUE,
  description              TEXT NOT NULL,
  parameters_json          JSON NOT NULL,              -- OpenAI function tools format
  sql_template             TEXT NOT NULL,              -- ? placeholder 로 인자 치환
  param_order_json         JSON,                       -- ? 순서대로 인자 이름 ["deviceId","days"]
  status                   ENUM('pending','approved','rejected','disabled') NOT NULL DEFAULT 'pending',
  source                   VARCHAR(50) NOT NULL DEFAULT 'llm-proposal', -- llm-proposal | manual
  rationale                TEXT,                       -- LLM 이 왜 이 도구를 제안했는지
  proposal_basis_json      JSON,                       -- 제안 근거 (audit 호출수, 예시 SQL 등)
  call_count               INT NOT NULL DEFAULT 0,     -- 활성화 후 호출 횟수
  last_called_at           DATETIME,
  created_at               DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  approved_at              DATETIME,
  approved_by              VARCHAR(64),
  INDEX idx_tool_registry_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
